...
DataDash is a Flask web app for analyzing arbitrary user-uploaded CSVs. The user-facing language is Spanish — keep UI strings, flash messages, and chart titles in Spanish when editing them.
...
No test suite, linter, or build step is configured. There is intentionally **no database** — adding one violates the project constraint.
...
2. **Pipeline** runs in this strict order — reorder at your peril:
   - `data_loader.load_csv` — encoding probe (utf-8 → utf-8-sig → latin-1 → cp1252), then `csv.Sniffer` for delimiter (`,;\t|`) with a heuristic fallback.
...
3. **Derivations**: `stats.numeric_summary`, `stats.dataset_overview`, `chart_builder.build_charts` (capped at `MAX_CHARTS = 12`).
...
- `GET /dashboard` — KPI cards, numeric summary table, charts grid, data table shell
...
- **Bootstrap 5 and Chart.js come from CDNs.** Don't vendor them locally.
